...
const URL_BASE = 'http://10.1.11.7/erudio/erudio-server/web/app_dev.php/api/alunos'
...
const criar = (recurso) => {
    return servidor.post(`${URL_BASE}/${recurso}`)
        .then(response => Promise.resolve(response.data))
        .catch(error => Promise.reject(error))
}

const atualizar = (recurso, id) => {
    return servidor.put(`${URL_BASE}/${recurso}/${id}`)
        .then(response => Promise.resolve(response.data))
        .catch(error => Promise.reject(error))
}

const remover = (recurso, id) => {
    return servidor.delete(`${URL_BASE}/${recurso}/${id}`)
