...
const URL_BASE = 'http://beta.erudio.itajai.sc.gov.br/servicos/web/api/alunos'
...
const getHeaders = () => {
    ...
    if (localStorage.getItem('token')) {
        ...
        headers['JWT-Authorization'] = 'Bearer ' + JWT_TOKEN
    }
    return headers
}
...
const criar = (recurso) => {
    return servidor.post(`${URL_BASE}/${recurso}`, {headers: getHeaders()})
        .then(response => Promise.resolve(response.data))
        .catch(error => Promise.reject(error))
}

const atualizar = (recurso, id) => {
    return servidor.put(`${URL_BASE}/${recurso}/${id}`, {headers: getHeaders()})
        .then(response => Promise.resolve(response.data))
        .catch(error => Promise.reject(error))
}
